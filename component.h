...
class lost{
private:
  char lostName[30],place[30],category[30]; // 遗失物名称、遗失地点
...
public:
    ...
    lost(string &name0,string &category0,string &place0,tm *t);
    lost();
    void verify();
    void pickUp(string stuName0,string phoneNumber0,string stuId0,tm pt);//这里是丢的部分
    string state2String();
    string type2String();
    string getLostDay();
    string getName();
